...
from networks.cifar10_nets import SimpleCNN
...
class CIFAR10Learner(Learner):  # also supports CIFAR10ScaffoldLearner
    def __init__(
        self,
        dataset_root: str = "/home/u00yjc00/NCKU_image_classification/data1/",
    ...
    def initialize(self, parts: dict, fl_ctx: FLContext):
        ...
        # Set dataset
        self.meta = pd.read_csv(self.dataset_root+"dataset.csv")
        
        self.train_dataset = self.train_input()
        self.valid_dataset = self.valid_input()
        print("loader")
        self.train_loader = torch.utils.data.DataLoader(self.train_dataset, batch_size=16, shuffle=True, num_workers=2)
        self.valid_loader = torch.utils.data.DataLoader(self.valid_dataset, batch_size=16, shuffle=False, num_workers=2)
    ...
    def train_input(self):
        self.data = self.meta[self.meta.process=="train"]
        self.train_dataset = []
        Y = []
        
        print("input")
        for n, y in zip(self.data.path, self.data.label):
            try:
                origin = np.asarray(Image.open(n).convert("RGB"), dtype=np.float32) / 255
                origin = origin[:, :, :3]
                self.train_dataset.append(origin)
                Y.append(y)
            except:
                pass
        Y = np.array(Y)
        train_loader = []
        OUTPUT_DIM = self.train_dataset[0].shape[0]
        for i in range(len(self.train_dataset)):
            train_loader.append([np.array(self.transform_train(torch.Tensor(self.train_dataset[i]).view(3, OUTPUT_DIM, OUTPUT_DIM))),Y[i]])
        print("input finish")
        return train_loader
    def valid_input(self):
        self.data = self.meta[self.meta.process=="valid"]
        self.valid_dataset = []
        Y = []
        for n, y in zip(self.data.path, self.data.label):
            try:
                origin = np.asarray(Image.open(n).convert("RGB"), dtype=np.float32) / 255
                origin = origin[:, :, :3]
                self.valid_dataset.append(origin)
                Y.append(y)
            except:
                pass
        Y = np.array(Y)
        valid_loader = []
        OUTPUT_DIM = self.valid_dataset[0].shape[0]
        for i in range(len(self.valid_dataset)):
            valid_loader.append([np.array(self.transform_valid(torch.Tensor(self.valid_dataset[i]).view(3, OUTPUT_DIM, OUTPUT_DIM))),Y[i]])
        return valid_loader
