...
from fastai.vision.all import verify_images, ImageDataLoaders, aug_transforms, Normalize, imagenet_stats, ToTensor, Resize, CrossEntropyLossFlat
...
class FastaiLearner(Learner):  # also supports CIFAR10ScaffoldLearner
    ...
    def initialize(self, parts: dict, fl_ctx: FLContext):
        ...
        self.meta = self.meta[["path","label"]]
        bad_path = verify_images(self.meta.path)
        self.meta = self.meta[~self.meta.path.isin(bad_path.items)]
        
        dls = ImageDataLoaders.from_df(self.meta, path="/", item_tfms = [ToTensor(), Resize(224), ]
                                            , batch_tfms=[*aug_transforms(do_flip=False, max_rotate = 0, max_zoom = 0,
                                                                         max_warp=0., p_affine=0,size=224),
